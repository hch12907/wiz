...
pub fn install_package(package: &Package, base_path: &Path) -> Result<(), String> {
    ...
    extract::extract_tar(&tarball_path, &gz_path)?;
    extract::extract_gz(&gz_path, &install_path)?;
    version::update_list(package, &base_path.to_path_buf().append(paths::INSTALLED_PACKAGE_LIST))?;
