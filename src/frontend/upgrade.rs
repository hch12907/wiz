...
fn upgrade_package(input: &str, base_path: &Path) -> Result<(), String> {
    for list_path in get!(fs::read_dir(base_path.to_path_buf().append(paths::PACKAGE_LIST_DIR)), "") {
        let latest = find::select_package(input, list_path.unwrap().path().as_path(), selection::select_package)?;
        for pkg in version::get_list(&base_path.to_path_buf().append(paths::INSTALLED_PACKAGE_LIST))? {
            if latest.name == pkg.name && latest.version > pkg.version {
                uninstallation::uninstall_package(&pkg, base_path)?;
                installation::install_package(&latest, base_path)?;
                return Ok(())
            }
        }
    }
